...
async def create_game(request: CreateGameRequest):
    ...
    if world_data:
        ...
        # 应用AI生成的地点
        if "locations" in world_data and world_data["locations"]:
            from models import Location
            game.world.locations = {}
            for loc_data in world_data["locations"]:
                loc = Location(
                    location_id=loc_data.get("location_id", generate_id()),
                    name=loc_data["name"],
                ...
                game.world.locations[loc.location_id] = loc
            game.world.current_location_id = list(game.world.locations.keys())[0]
        
        # 应用AI生成的NPC
        if "npcs" in world_data and world_data["npcs"]:
            from models import NPC
            game.world.npcs = {}
            for npc_data in world_data["npcs"]:
                npc = NPC(
                    npc_id=npc_data.get("id", generate_id()),
                    name=npc_data["name"],
                    role=npc_data.get("role", "npc"),
                    disposition=npc_data.get("disposition", "neutral"),
                    appearance=npc_data.get("appearance", ""),
                    personality=npc_data.get("personality", ""),
                    location=game.world.current_location_id,
                    dialogue_style=npc_data.get("voice_style", "neutral"),
                    knowledge=npc_data.get("backstory", "").split("。")[:2]
                )
                game.world.npcs[npc.npc_id] = npc
